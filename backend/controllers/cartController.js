...
exports.getById = async (req, res, next) => {
  try {
    const products = await cartService.getById(req.params.id);
    res.json(products);
  } catch (error) {
    return res.status(500).send({ success: false, msg: error.message });
  }
...
exports.update = async (req, res, next) => {
  try {
    const isExists = await cartService.getById(req.params.id);
    if (isExists) {
      ...
      const result = await cartService.update(req.params.id, isExists);
